...
ref_for_rmsc = dict(project='ref_ts', product='HadISST', period='1990-2010', dataset_type='obs_reference',
                    frequency='monthly', variable='tos')
all_dataset_dicts = Wmodels + reference_models + reference_datasets
#
results = dict()
variable = 'tos'
# outjson = main_cesmep_path + '/' + opts.comparison + '/TuningMetrics/' + variable + '_' + opts.comparison +
#           '_metrics_over_regions_for_tuning.json'
...
for dataset_dict in all_dataset_dicts:
    ...
    if 'customname' in wdataset_dict:
        ...
        if 'clim_period' in wdataset_dict:
            wperiod = wdataset_dict['clim_period']
        if 'period' in wdataset_dict:
            if wdataset_dict['period'] not in 'fx':
                wperiod = wdataset_dict['period']
    ...
    #
    # -- Loop on the regions; build the results dictionary and save it in the json file
    # variable_comparison_spatial_averages_over_regions.json
    ...
    for region in regions_for_spatial_averages:
        ...
        if 'product' in wdataset_dict:
            if wdataset_dict['product'] == 'WOA13-v2':
                rawvalue = rawvalue[0]
        ...
        if wdataset_dict['dataset_type'] != 'obs_reference':
            ...
            if safe_mode:
                try:
                    rmsc = cMA(ccdo(time_average(space_average(ccdo(minus(anom_dat, anom_ref), operator='sqr'))),
                                    operator='sqrt'))[0][0][0]
                    rms = cMA(ccdo(time_average(space_average(ccdo(minus(scyc_dat, scyc_ref), operator='sqr'))),
                                   operator='sqrt'))[0][0][0]
                except:
...
for region in regions_for_spatial_averages:
    # -- plot the raw values
    figname = atlas_dir + '/' + opts.comparison + '_' + variable + '_' + region['region_name'] + \
              '_rawvalues_over_regions_for_tuning.png'
    cmd = 'Rscript --vanilla ' + main_cesmep_path + \
          'share/scientific_packages/TuningMetrics/plot_rawvalue.R --metrics_json_file ' + outjson + ' --region ' + \
          region['region_name'] + ' --figname ' + figname
    print(cmd)
    os.system(cmd)
    ...
    # -- plot the rmsc
    figname = atlas_dir + '/' + opts.comparison + '_' + variable + '_' + region['region_name'] + \
              '_rmsc_over_regions_for_tuning.png'
    cmd = 'Rscript --vanilla ' + main_cesmep_path + \
          'share/scientific_packages/TuningMetrics/plot_rmsc.R --metrics_json_file ' + outjson + ' --region ' + \
          region['region_name'] + ' --figname ' + figname
    print(cmd)
    os.system(cmd)
    index += cell("", os.path.basename(figname), thumbnail="700*600", hover=False)
    ...
    # -- plot the rms
    figname = atlas_dir + '/' + opts.comparison + '_' + variable + '_' + region['region_name'] + \
              '_rms_over_regions_for_tuning.png'
    cmd = 'Rscript --vanilla ' + main_cesmep_path + \
          'share/scientific_packages/TuningMetrics/plot_rmsc.R --metrics_json_file ' + outjson + ' --region ' + \
          region['region_name'] + ' --figname ' + figname + ' --statistic rms'
    print(cmd)
    os.system(cmd)
    index += cell("", os.path.basename(figname), thumbnail="700*600", hover=False)
