...
for (tmp in names(results)){
    if (tmp!='json_structure' & tmp!='variable'){
        ...
        if (results[[tmp]]$dataset_dict$dataset_type=='reference_dataset'){
            reference_dataset = c(reference_dataset, as.numeric(results[[tmp]]$results[[region]]$bias))
        ...
        if (results[[tmp]]$dataset_dict$dataset_type=='test_dataset'){
            test_dataset = c(test_dataset, as.numeric(results[[tmp]]$results[[region]]$bias))
...
dumylim = max(abs(range(c(reference_dataset,test_dataset),na.rm=TRUE)))
ylim=c(-dumylim,dumylim)
ylim = c(-1.5,1.5)
print("ylim = ")
print(ylim)
