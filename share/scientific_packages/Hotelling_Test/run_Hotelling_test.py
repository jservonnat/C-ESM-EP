...
parser.add_option("--reference_results",
                  help="Choose between CMIP5 and AMIP (default is CMIP5) to add the results of CMIP5 or AMIP to the plot)",
                  action="store",default=None)
...
parser.add_option("--common_space",
                   help='Common EOFs space used to compute the Hotelling test; choose between CCM (for Common Covariance Matrix) and CMR (Covariance Matrix of the Reference mean)',
...
if opts.reference_results: reference_results = opts.reference_results
...
# Les figures des resultats des statistics vont directement dans le repertoire de la comparaison: ils sont directement lies a la comparaison definie
...
for variable in variables:
    ...
    for model in Wmodels:
        ...
        output_res_hotelling_json_file = hotelling_outputdir+'results_json_files/Res-Hotelling_'+dataset_name_in_filename+'-'+variable+'.json'
        #
        # -- Add the path to the netcdf file
        wmodel.update(dict(file=cfile(dat), variable=variable, dataset_name_in_filename=dataset_name_in_filename, output_res_hotelling_json_file=output_res_hotelling_json_file))
        if os.path.isfile(output_res_hotelling_json_file) and not force_compute==True:
    ...
    fp.close()
    #
    cmd = 'Rscript --vanilla main_Hotelling.R --test_json_file '+TestFileName+' --reference_json_file '+RefFileName+' --outputdir '+hotelling_outputdir
    print cmd
    ...
    cmd = 'Rscript --vanilla Plot-Hotelling-test-results-one-variable.R --test_json_files '+TestFileName+' --figname '+figname
    print cmd
    p=subprocess.Popen(shlex.split(cmd))
    p.communicate()
