...
def GPMultiRegions(df,
    ...
    Exclude = ['simulationInputClimatologyFileCheckSum', 'simulationTrackingID', 'referenceFileCheckSum',
               'referenceFilename', 'simulationInputClimatologyFileName', 'referenceTrackingDate',
    ...
    if len(gtone) and not ForceAggregation:
    ...
    if len(refsimulations) != len(df) and not displayTestOnly:
    ...
    for tag in scorehtmlsoup.findAll('td'):
        ...
        if not math.isnan(float(v)):
            tag['style'] = "font-family: monospace; font-size:10px; text-align: center; background-color:rgb" + \
                           cmap2rgb(float(v))
...
def GlecklerPlot(df, MetricName='rms_xyt', dataType='ann', Region='GLB', displayTestOnly=True, SortBy=None,
                 ...
                 Index=['simulationActivity', 'simulationInstitute', 'simulationModel', 'simulationExperiment',
                        'simulationName', 'simulationPeriod', 'simulationLogin'],
                 Normalization=None, ByColumnsOrRows='Columns', MaxValue=None, ForceAggregation=False,
                 html_pathandfilename=None, ndigits=".2f", cellWidth="100px", displayMeanOverReference=True):
    ...
    if MetricName in ['bias_xy', 'rms_xy', 'rms_xyt', 'cor_xyt', 'cor_xy', 'mae_xy']:
        if Normalization == 'NormByStandardDeviation' and ByColumnsOrRows == 'Columns':
            ...
            scoreRefSimulations = (scoreRefSimulations - scoreRefSimulations.mean(axis=0)) / \
                                   scoreRefSimulations.std(axis=0)
            print '# --- Display normalized results ; by column ;'
