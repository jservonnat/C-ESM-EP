...
def split_periods_in_models(models):
    ...
    for model in Wmodels:
        splitted_model = []
        if 'frequency' in model:
            if model['frequency'] in ['seasonal', 'annual_cycle']:
                wperiod = 'clim_period'
            if model['frequency'] in ['monthly']:
                wperiod = 'period'
...
def build_metric_outpath(model, group, subdir=None, root_outpath=None):
    ...
    #
    return root_outpath + '/PMP_OUT/metrics_results/' + subdir + '/raw/' + project + '_' + login + '_' + model + '_' + \
           experiment + '_' + simulation + '_' + period + '_' + group
