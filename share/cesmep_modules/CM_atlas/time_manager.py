...
def find_common_period(models, common_period_variable, common_clim_period):
   ...
   for model in models:
       ...
       if 'clim_period' in model:
          if model['clim_period']=='common_clim_period':
             ...
             cp_model = ''
             if 'model' in wmodel: cp_model = wmodel['model']+' '
             if 'simulation' in wmodel: cp_model = cp_model + wmodel['simulation']
             common_period_models.append(cp_model)
             if model['frequency']=='monthly' : wperiod = wmodel['period']
             if model['frequency']=='seasonal': wperiod = wmodel['clim_period']
             print wmodel
             print wperiod
   ...
   for model in models:
       ...
       print model
   #
   print '==> Model with earliest last available clim_period = '+common_period_models[startyear_last_available_periods.index(sorted_periods[0])]
   return models
