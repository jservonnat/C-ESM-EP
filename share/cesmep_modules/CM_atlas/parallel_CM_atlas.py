    ...
    if not do_cfile:
       ...
       if not safe_mode:
          ...
          try:
             ...
             print '--> Successfully plotted ',myplot,' with plotname ',plot_filename
...
def plot_climato(var, dat, season, proj='GLOB', domain={}, custom_plot_params={}, do_cfile=True, mpCenterLonF=None,
    ...
    print 'var in plot_climato = ',var
    print 'dat in plot_climato = ',dat
    grid = None
    table = None
    realm = None
    scale = 1.
    ...
    print 'wdat before frequency_manager_for_diag = ',wdat
    frequency_manager_for_diag(wdat, diag='SE')
    print 'wdat before get_period_manager = ',wdat
    get_period_manager(wdat)
    # -- Get the dataset
    ds_dat = ds(**wdat)
    #print 'ds_dat',ds_dat
...
#Parallel(n_jobs=num_cores)(delayed(safe_mode_cfile_plot)(climaf_crs) for climaf_crs in plots_crs)
    ...
    for var in variables:
        ...
        for wref in var_references:
           ...
           for model in models:
               ...
               if do_cfile:
                  indiv_end = datetime.utcnow()
                  duration = indiv_end - indiv_start
                  print 'Time needed between call of plot_diff with do_cfile=True and obtaining result: ',model_diff
                  print '----> ',duration.seconds,'seconds'
                  tmp_crs = plot_diff(var, wmodel, ref, season, proj=proj, domain=domain, custom_plot_params=custom_plot_params,
                                      ocean_variables=ocean_variables,
                                      safe_mode=safe_mode, add_product_in_title=add_product_in_title, shade_missing=shade_missing,
                                      apply_period_manager=apply_period_manager, do_cfile=False)
                  print 'Here is the CRS: ',tmp_crs.crs
                  print ''
...
#except:
#  print 'joblib and multiprocessing are not available; '

def parallel_section_2D_maps(models, reference, proj, season, variables, section_title, domain,
                    safe_mode=True, add_product_in_title=True, shade_missing=False, zonmean_variable=False, ocean_variables=ocean_variables,
                    custom_plot_params={}, custom_obs_dict={}, alternative_dir={}, add_line_of_climato_plots=False, thumbnail_size=None,
                    apply_period_manager=True, do_cfile=True, num_cores=None):

    start = datetime.utcnow()

    # -- Build the list of plot CRS we will execute in parallel with do_cfile=True    
    plots_crs = section_2D_maps(models, reference, proj, season, variables, section_title, domain,
                    safe_mode=safe_mode, add_product_in_title=add_product_in_title, shade_missing=shade_missing, zonmean_variable=zonmean_variable, ocean_variables=ocean_variables,
                    custom_plot_params=custom_plot_params, custom_obs_dict=custom_obs_dict, alternative_dir=alternative_dir,
                    add_line_of_climato_plots=add_line_of_climato_plots, thumbnail_size=thumbnail_size,
                    apply_period_manager=apply_period_manager, do_cfile=False)
    end = datetime.utcnow()
    duration = end - start
    print 'Builing plot_crs done in :',duration.seconds,'seconds'
    #
    ...
    print 'files computed in parallel = ',r
    end = datetime.utcnow()
    duration = end - start
    print 'Parallel Atlas Explorer computation done in :',duration.seconds,'seconds'
    #
    from climaf.cache import csync
    print '--> Synchronize the cache after parallel computation'
    csync(True)
    #print 'Trying climaf.cache.rebuild'
    #rebuild()
    #start = datetime.utcnow()
    #import time
    #time.sleep(120)
    #if os.path.isfile('/prodigfs/ipslfs/dods/jservon/climafcache_ParallelAtlasExplorer/c2d89/d55aa/99908/e697c/39f88/f07e6/3120a/42d46/44e11/a42f6/63659/d.png'):
    #   print 'Yes I found /prodigfs/ipslfs/dods/jservon/climafcache_ParallelAtlasExplorer/c2d89/d55aa/99908/e697c/39f88/f07e6/3120a/42d46/44e11/a42f6/63659/d.png'
    #else:
    #   print 'Fuck no, /prodigfs/ipslfs/dods/jservon/climafcache_ParallelAtlasExplorer/c2d89/d55aa/99908/e697c/39f88/f07e6/3120a/42d46/44e11/a42f6/63659/d.png does not exist yet'
    #if os.path.isfile('/prodigfs/ipslfs/dods/jservon/climafcache_ParallelAtlasExplorer/1e0fb/c863b/c2945/ae85f/7bf84/93767/2c9c3/0b166/de013/3b9cb/b7c45/0.png'):
    #   print 'Ok for this one'
    #else:
    #   print 'Fuck no it doesnt'
    #if os.path.isfile('/prodigfs/ipslfs/dods/jservon/climafcache_ParallelAtlasExplorer/index'):
    #   print 'CliMAF index exists'
    #   test_index = open('/prodigfs/ipslfs/dods/jservon/climafcache_ParallelAtlasExplorer/index')
    #   for line in test_index.readlines():
    #       print line
    #else:
    #   print 'Noooo CliMAF index doesnt exist'
    #for climaf_crs in plots_crs:
    #    print 'climaf_crs ', climaf_crs
    #    print 'cfile(climaf_crs): ', cfile(climaf_crs)
    #    print ' - '
    #end = datetime.utcnow()
    #duration = end - start
    #print 'cfile done on all climaf_crs done in :',duration.seconds,'seconds'
    #start = datetime.utcnow()
    #for climaf_crs in plots_crs:
    #    print 'climaf_crs ', climaf_crs
    #    print 'cfile(climaf_crs): ', cfile(climaf_crs)
    ...
    #end = datetime.utcnow()
    #duration = end - start
    #print 'Second round of cfile done in :',duration.seconds,'seconds'
    # --> Now the plots are in the cache; we simply re-invoke section_2D_maps to build the html page
    #     using the existing plots
    #
    #return ''
    return section_2D_maps(models, reference, proj, season, variables, section_title, domain,
                    safe_mode=safe_mode, add_product_in_title=add_product_in_title, shade_missing=shade_missing, zonmean_variable=zonmean_variable, ocean_variables=ocean_variables,
                    custom_plot_params=custom_plot_params, custom_obs_dict=custom_obs_dict, alternative_dir=alternative_dir,
                    add_line_of_climato_plots=add_line_of_climato_plots, thumbnail_size=thumbnail_size,
                    apply_period_manager=apply_period_manager, do_cfile=True)
