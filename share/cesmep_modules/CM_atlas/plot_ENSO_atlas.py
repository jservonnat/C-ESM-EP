...
def ENSO_std_ssta(dat_dict, do_cfile=True, safe_mode=True, custom_plot_params = {}):
    ...
    #
    # -- Compute the SST anomalies (dat - annual cycle)
    #ssta = llbox(regridn(ccdo(minus(dat,scyc),operator='detrend'),cdogrid='r180x90'),lonmin=120,lonmax=290,latmin=-30,latmax=30)
    std_ssta = llbox( regridn( ccdo( ccdo(minus(dat,scyc),operator='detrend'), operator='timstd'), cdogrid='r360x180'), lonmin=120,lonmax=290,latmin=-30,latmax=30)
