...
def section_zonalmean_slices(models=[], reference='default', zonmean_slices_variables=[], zonmean_slices_basins=[],
                             zonmean_slices_seas=[],
    ...
    if do_parallel:
        ...
        index = section(
            "Zonal Means Sections per ocean basin --> Model regridded on reference (before computing the zonal mean)",
            level=4)
        do_cfile = True
    #
    # Loop over variables
        ...
        for season in zonmean_slices_seas:
            ...
            for basin in zonmean_slices_basins:
                ...
                basin_zonmean_modelgrid = zonal_mean_slice(ref, variable, basin=basin, season=season,
                                                           ...
                                                           method='regrid_model_on_ref')
                if not do_cfile:
                    plots_crs.append(basin_zonmean_modelgrid)
                else:
                    index += cell("", basin_zonmean_modelgrid, thumbnail=thumbsize_zonalmean, hover=hover,
                                  **alternative_dir)
                for model in models:
                    basin_zonmean_modelgrid = zonal_mean_slice(model, variable, basin=basin, season=season,
                                                               ref=ref, safe_mode=safe_mode, do_cfile=do_cfile, y=y,
                                                               add_product_in_title=None,
                                                               custom_plot_params=custom_plot_params,
                                                               method='regrid_model_on_ref')
                    if not do_cfile:
