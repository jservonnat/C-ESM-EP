...
if atlas_head_title is None :
    atlas_head_title = "Ocean main diags"
...
if do_MLD_maps:
    ...
    if not MLD_diags:
        MLD_diags = [('ANM', 'GLOB'), ('JFM', 'GLOB'), ('JAS', 'GLOB'), ('JFM', 'NH40'), ('Annual Max', 'NH40'),
                     ('JAS', 'SH30'), ('Annual Max', 'SH30')]
    #
    # -- Period Manager
    if not use_available_period_set:
    ...
    if not curl_diags:
        curl_diags = [dict(name='Global, annual mean', season='ANM', proj='GLOB', thumbNsize='400*300'),
                      ...
                      dict(name='Tropical Atlantic, annual mean', season='ANM',
                           domain=dict(lonmin=-90, lonmax=0, latmin=-10, latmax=45), thumbNsize='400*300'),
                      dict(name='North Pacific, annual mean', season='ANM',
                           domain=dict(lonmin=120, lonmax=240, latmin=30, latmax=75), thumbNsize='500*250'),
                      dict(name='North Atlantic, JFM', season='JFM',
                           domain=dict(lonmin=-80, lonmax=0, latmin=30, latmax=90), thumbNsize='400*300'),
                      ]
        ...
        for region in ts_basins:
            ...
            for model in Wmodels:
                if variable == "tos" and region == "GLO":
                    ...
                    basin_index = index_timeserie(model, variable, region=region, obs=hadisst_ts, prang=None,
                                                  safe_mode=safe_mode)
                if variable == "zos" and region == "GLO":
                    print("=> comparisin with AVISO-L4")
                    basin_index = index_timeserie(model, variable, region=region, obs=aviso_ts, prang=None,
                                                  safe_mode=safe_mode)
                else:
    ...
    for variable in VertProf_variables:
        for obs in VertProf_obs:
            for region in VertProf_basins:
                # -- Line title
                index += start_line(title_region(region) + ' ' + varlongname(variable) + ' (' + variable + ') vs '
                                    + obs.get("product"))
                for model in Wmodels:
                    if region == "GLO":
