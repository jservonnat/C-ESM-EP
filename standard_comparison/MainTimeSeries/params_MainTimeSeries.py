...
def annual_north_seaice_volume(dat, **kwargs):
    if 'gridfile' in kwargs:
        ds_grid = fds(kwargs['gridfile'], period='fx', variable='area')
    else:
        print('No grid file provided')
    return minus(
            ccdo(
                multiply(
                    ccdo(dat, operator='yearmean'),
                    ds_grid
                ),
                operator='fldsum'),
            annual_south_seaice_volume(dat,**kwargs)
            )

def annual_north_seaice_extent(dat, **kwargs):
    if 'gridfile' in kwargs:
        ds_grid = fds(kwargs['gridfile'], period='fx', variable='area')
    else:
        print('No grid file provided')
    return minus(
            ccdo(
                multiply(
                    ccdo(ccdo(dat,operator='gec,.15'), operator='yearmean'),
                    ds_grid
                ),
                operator='fldsum'),
            annual_south_seaice_extent(dat,**kwargs)
            )

def annual_south_seaice_extent(dat, **kwargs):
    domain = dict(lonmin=0, lonmax=360, latmin=-90, latmax=-50)
    if 'gridfile' in kwargs:
        ds_grid = fds(kwargs['gridfile'], period='fx', variable='area')
    else:
        print('No grid file provided')
    return ccdo(
        multiply(
            llbox(ccdo(ccdo(dat,operator='gec,.15'), operator='yearmean'), **domain),
...
def annual_seaice_volume(dat, **kwargs):
    domain = dict(lonmin=0, lonmax=360, latmin=-90, latmax=90)
    if 'gridfile' in kwargs:
        ds_grid = fds(kwargs['gridfile'], period='fx', variable='area')
    else:
        print('No grid file provided')
...
def annual_south_seaice_volume(dat, **kwargs):
    domain = dict(lonmin=0, lonmax=360, latmin=-90, latmax=-50)
    if 'gridfile' in kwargs:
        ds_grid = fds(kwargs['gridfile'], period='fx', variable='area')
    else:
...
def Sep_north_seaice_extent(dat, **kwargs):
    if 'gridfile' in kwargs:
        ds_grid = fds(kwargs['gridfile'], period='fx', variable='area')
    else:
        print('No grid file provided')
    return minus(
            ccdo(
                multiply(
                    ccdo(ccdo(dat,operator='gec,.15'), operator='selsmon,9'),
                    ds_grid
                ),
                operator='fldsum'),
            annual_south_seaice_extent(dat,**kwargs)
...
def FebMar_south_seaice_extent(dat, **kwargs):
    domain = dict(lonmin=0, lonmax=360, latmin=-90, latmax=-50)
    if 'gridfile' in kwargs:
        ds_grid = fds(kwargs['gridfile'], period='fx', variable='area')
    else:
    ...
    return ccdo(
        multiply(
            llbox(ccdo(ccdo(ccdo(dat,operator='gec,.15'), operator='selseason,FM'),operator='yearmean'), **domain),
            llbox(ds_grid, **domain)
        ),
        operator='fldsum')

def FebMar_north_seaice_extent(dat, **kwargs):
    if 'gridfile' in kwargs:
        ds_grid = fds(kwargs['gridfile'], period='fx', variable='area')
    else:
        print('No grid file provided')
    return minus(
            ccdo(
                multiply(
                    ccdo(ccdo(ccdo(dat,operator='gec,.15'), operator='selseason,FM'),operator='yearmean'),
                    ds_grid
                ),
                operator='fldsum'),
            annual_south_seaice_extent(dat,**kwargs)
...
def Sep_south_seaice_extent(dat, **kwargs):
    domain = dict(lonmin=0, lonmax=360, latmin=-90, latmax=-50)
    if 'gridfile' in kwargs:
        ds_grid = fds(kwargs['gridfile'], period='fx', variable='area')
    else:
    ...
    return ccdo(
        multiply(
            llbox(ccdo(ccdo(dat,operator='gec,.15'), operator='selsmon,9'), **domain),
            llbox(ds_grid, **domain)
        ),
        operator='fldsum')


def AMOC_finder(dat,**kwargs):
    #cscript('latitude_extractor','ncks -v nav_lat ${in} ${out}', _var='nav_lat')
    #lat_grid=latitude_extractor(dat)
    ...
    dat_deep_yearly=ccdo(cslice_select(dat,dim='depthw', min=500., max=10000.),operator='yearmean')
    dat_windowed=ccdo(ccdo2(latitude_filter, dat_deep_yearly, operator='ifthen'),operator='fldmax')
    cscript('max_msft_atl','cdo vertmax ${in} ${out}', _var='msftyz_atl')
    return max_msft_atl(dat_windowed)
...
time_series_specs = [
    ...
    dict(variable='sivolu',
         project_specs=dict(
             IGCM_OUT=dict(DIR='ICE')
         ),
         operation_kwargs = dict(),
         operation=annual_north_seaice_volume,
         scale=1/1e9,
    ...
    dict(variable='sivolu',
         ...
         operation_kwargs = dict(),
         operation=annual_south_seaice_volume,
         scale=1/1e9,
         center_string='Antarctic Sea Ice Annual Volume',
         right_string='ANM',
         ylabel='Sea Ice Volume (km3)', xlabel='Time (years)',
         ylabel_fontsize=15,
        ),
...
#             IGCM_OUT=dict(DIR='ICE')
#         ),
#         operation_kwargs = dict(grid_file='/home/ggachon/eORCA1.4.2_grid.nc'),
#         operation=annual_north_seaice_volume,
#         scale=1/1e8,
#         center_string='Arctic Sea Ice Annual Volume',
#         right_string='-',
...
#         ylabel_fontsize=15,
#        )
    dict(variable='siconc',
         project_specs=dict(
             IGCM_OUT=dict(DIR='ICE')
         ),
         operation_kwargs = dict(),
         operation=annual_north_seaice_extent,
         scale=1/1e12,
         center_string='Arctic Sea Ice Annual Extent',
         right_string='ANM',
         ylabel='Sea Ice Area (1e6 km2)', xlabel='Time (years)',
         ylabel_fontsize=15,
        ),
    ...
    dict(variable='siconc',
         ...
         operation_kwargs = dict(),
         operation=annual_south_seaice_extent,
         scale=1/1e12,
         center_string='Antarctic Sea Ice Annual Extent',
         right_string='ANM',
         ylabel='Sea Ice Area (1e6 km2)', xlabel='Time (years)',
         ylabel_fontsize=15,
        ),
#
    dict(variable='siconc',
         ...
         operation_kwargs = dict(),
         operation=FebMar_north_seaice_extent,
         scale=1/1e12,
         center_string='Arctic Sea Ice Winter Extent',
         right_string='Feb+Mar',
         ylabel='Sea Ice Area (1e6 km2)', xlabel='Time (years)',
         ylabel_fontsize=15,
        ),
    ...
    dict(variable='siconc',
         ...
         operation_kwargs = dict(),
         operation=Sep_south_seaice_extent,
         scale=1/1e12,
         center_string='Antarctic Sea Ice Winter Extent',
         right_string='Sep',
         ylabel='Sea Ice Area (1e6 km2)', xlabel='Time (years)',
         ylabel_fontsize=15,
        ),
#
    dict(variable='siconc',
         ...
         operation_kwargs = dict(),
         operation=Sep_north_seaice_extent,
         scale=1/1e12,
         center_string='Arctic Sea Ice Summer Extent',
         right_string='Sep',
         ylabel='Sea Ice Area (1e6 km2)', xlabel='Time (years)',
         ylabel_fontsize=15,
        ),
    ...
    dict(variable='siconc',
         ...
         operation_kwargs = dict(),
         operation=FebMar_south_seaice_extent,
         scale=1/1e12,
         center_string='Antarctic Sea Ice Summer Extent',
         right_string='Feb+Mar',
         ylabel='Sea Ice Area (1e6 km2)', xlabel='Time (years)',
         ylabel_fontsize=15,
        ),
    ...
    dict(variable='msftyz_atl',
         project_specs=dict(
             IGCM_OUT=dict(DIR='OCE')
         ), 
         operation_kwargs = dict(),
         operation=AMOC_finder,
         center_string='AMOC',
         right_string='ANM',
         ylabel='NADW (Sv)', xlabel='Time (years)',
         ylabel_fontsize=15,
        )
