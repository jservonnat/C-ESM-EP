...
def average_over_region(dat, region_number=None, regions_file=None,
    ...
    if regions_file is None or region_number is None :
        ...
        if contfrac is not None:
            # Compute area weighted field integral of variable * cont.fraction area over the region
            var_integral = ccdo3(dat, contfrac, region_mask, operator="fldint -mul -mul")
            if fldmean == "fldint":
                return var_integral            
            elif fldmean == "fldmean":
                # Compute area weighted field integral of continental fraction over the region
...
derive("IGCM_OUT", 'pr', 'ccdo2' , 'rain', 'prsn', operator='add')
