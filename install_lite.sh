...
# refering (through symbolic links and PYTHONPATH) to the code in current dir
#   - First argument: the directory that will host C-ESM-EP (in a subdir called cesmep_lite)
...
(cd $o ; tar -cf - --exclude=*.out --exclude=*~ --exclude=climaf.log \
	 --exclude=job.in  $comparison) | \
    (cd $target; tar -xf - )
