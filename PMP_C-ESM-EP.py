...
parser.add_option("--CMIP5_highlights",
                  help="Names of the CMIP5 models that will nbe highlighted "
                       "(separated with commas: IPSL-CM5A-LR,CNRM-CM5))",
                  action="store", default=None)
parser.add_option("--CMIP5_highlights_first",
                  help="If True, we sort the results on the first model provided as highlight. If False, we use the "
                       "first dataset provided in datasets_setup.py .",
                  action="store", default=None)
...
parser.add_option("--root_outpath",
                  help="Root of the output directory for the climatologies (netcdf files) used by the PMP to compute "
                       "the metrics",
                  action="store", default=None)
...
#                 'mediumseagreen','violetred2','mediumturquoise','cadetblue','brown2','chartreuse1','burlywood3',
#                 'coral1','burlywood4','darkgoldenrod2','darkolivegreen3','darkgoldenrod4','darkorchid']
...
if opts.legend_ratio:
    legend_ratio = opts.legend_ratio
if opts.sort:
    sort = opts.sort
if opts.CMIP5_colors:
    CMIP5_colors = opts.CMIP5_colors
if opts.CMIP5_highlights:
    CMIP5_highlights = str.split(opts.CMIP5_highlights, ',')
if opts.highlights_only:
    highlights_only = opts.highlights_only
if opts.lwd_background:
    lwd_background = opts.lwd_background
if opts.lwd_highlights:
    lwd_highlights = opts.lwd_highlights
if opts.outfigdir:
    outfigdir = opts.outfigdir
if opts.root_outpath:
    root_outpath = opts.root_outpath
if opts.reference:
    reference = opts.reference
if opts.ref_parallel_coordinates:
    ref_parallel_coordinates = opts.ref_parallel_coordinates
if opts.reference_data_path:
    reference_data_path = opts.reference_data_path
...
if not reference_data_path:
    if ref_parallel_coordinates in 'CMIP5':
        reference_data_path = '/data/jservon/Evaluation/metrics_results/CMIP_metrics_results/CMIP5_20161103/' \
                              'cmip5clims_metrics_package-historical/03Nov2016'
        # reference_data_path = main_cesmep_path+'scientific_packages/PCMDI_CMIP_metrics_results/CMIP5_20161103/' \
        #                       'cmip5clims_metrics_package-historical/03Nov2016'#'/data/jservon/Evaluation/' \
        #                       'metrics_results/CMIP_metrics_results/CMIP5_20161103/' \
        #                       'cmip5clims_metrics_package-historical/03Nov2016'
    if ref_parallel_coordinates in 'AMIP':
        reference_data_path = '/data/jservon/Evaluation/metrics_results/CMIP_metrics_results/CMIP5_20161103/' \
                              'cmip5clims_metrics_package-amip/03Nov2016'
...
#         => on verifie si on a pas deja les fichiers json dans les repertoires
#            (faire une fonction pour construire le path)
...
for wmodel in Wmodels:
    ...
    for var_dict in vars:
        ...
        if 'Ok':
            ...
            found = False
            for group in groups:
                check_json = build_metric_outpath(wmodel, group, root_outpath=root_outpath,
                                                  subdir='*') + '/' + str.replace(var, '_', '-') + metric_json_filename
    ...
    for model_dict in Wmodels:
        ...
        if force_compute_metrics:
            ...
            for variable in vars:
                ...
                metric_json_file = metrics_output_path + '/' + w_variable + '_' + targetGrid + \
                                   '_esmf_linear_metrics.json'
        ...
        if w_variables:
            for var in w_variables:
                ...
                try:
                    ...
                    if wmodel_dict['project'] == 'MGV':
                        ...
                        cmd = 'ncrename -v t_ave_02592000,time -d t_ave_02592000,time ' + input_climatologies_dir + '/'\
                              + target_filename
                        print cmd
                        os.system(cmd)
                ...
                metric_files_list.append(metrics_output_path + '/' + str.replace(strvar, '_', '-') +
                                         '_' + targetGrid + '_esmf_linear_metrics.json')
            ...
            if not os.path.isdir(tmp_paramfiles_dir):
                os.makedirs(tmp_paramfiles_dir)
            tmp_paramfile = tmp_paramfiles_dir + os.path.basename(template_paramfile).replace('.py',
                                                                                              '_' + additionnal + '.py')
            ...
            if 'period' in keys_wmodel:
                if keys_wmodel['period'] == 'fx':
                    if 'clim_period' in keys_wmodel:
                        keys_wmodel['period'] = keys_wmodel['clim_period']
    ...
    if 'period' in wmodel:
        if wmodel['period'] == 'fx':
            if 'clim_period' in wmodel:
                wmodel['period'] = wmodel['clim_period']
    #      
...
# -- BLOCK 7 -------------------------------------------------------------------
cmd_parallel_coordinates = 'Rscript ' + parallel_coordinates_script + ' --test_data_path ' + test_data_path + \
...
if not metrics_sections:
    metrics_sections = [
        dict(statistic=['rms_xyt', 'rmsc_xy', 'bias_xy', 'cor_xy'],
             ...
             season=['ann'],
             section_name='Global, land, ocean on annual cycle and annual mean '
                          '(rms_xyt, rmsc_xyt, bias_xy and cor_xy)'),
        dict(statistic=['rms_xy', 'rmsc_xy', 'bias_xy', 'cor_xy'],
             region=['NHEX', 'TROPICS', 'SHEX'],
             season=['ann', 'djf', 'mam', 'jja', 'son'],
             section_name='NHEX (90N-20N), TROPICS (20N-20S), and SHEX (20S-90S), annual and seasonal means '
                          '(rms_xy, rmsc_xy, bias_xy and cor_xy)'),
    ]
        ...
        for region in metrics_section['region']:
            for season in metrics_section['season']:
                ...
                for statistic in metrics_section['statistic']:
                    ...
                    delay = datetime.utcnow() - datetime(2015, 1, 1)
                    nb = str(delay.microseconds)
                    figname = statistic + '_' + season + '_' + region + '_' + comparison + '_parallel_coordinates_' + \
                              nb + '.png'
                    cmd_parallel_coordinates_final = cmd_parallel_coordinates + ' --figname ' + outfigdir + figname + \
                                                     ' --statistic ' + statistic + ' --region ' + region + ' --season '\
                                                     + season
    ...
    for metrics_section in metrics_sections:
        ...
        for region in metrics_section['region']:
            ...
            for statistic in metrics_section['statistic']:
                ...
                delay = datetime.utcnow() - datetime(2015, 1, 1)
                nb = str(delay.microseconds)
                figname = statistic + '_tas_pr_4_seasons_' + region + '_' + comparison + '_parallel_coordinates_' + nb \
                          + '.png'
                cmd_parallel_coordinates_final = cmd_parallel_coordinates + ' --figname ' + outfigdir + figname + \
